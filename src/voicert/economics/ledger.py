...
class BudgetPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if self.degrade_to not in self.allowed_tiers and self.degrade_to != CostTier.CANNED:
            raise BudgetMisuseError(
                f"degrade_to={self.degrade_to.name} is not in allowed_tiers"
            )
...
class SessionLedger:
    ...
    def remaining_nusd(self) -> NanoUSD:
        """The binding constraint: whichever of session or lifetime is tighter."""
        with self.player._lock:
            session_left = max(
                0, self.policy.per_session_ceiling_nusd - self._committed_locked()
            )
        return min(session_left, self.player.lifetime_remaining_nusd)
    ...
    def authorize(
        ...
        if plan.headline not in self.policy.allowed_tiers:
            return self._deny(
                DenialReason.TIER_UNAVAILABLE, "policy", estimate, usage, 0
            )

        with self.player._lock:
            if estimate.total_nusd > self.policy.per_turn_ceiling_nusd:
                return self._deny(
            ...
            if estimate.total_nusd > session_left:
                return self._deny(
                    DenialReason.SESSION_CEILING, "session", estimate, usage, session_left
                )
            lifetime_left = max(
                0,
                self.policy.lifetime_ceiling_nusd - self.player._committed_locked(),
            )
            if estimate.total_nusd > lifetime_left:
                return self._deny(
    ...
    def settle(self, auth: Authorization, actual: TurnUsage | None = None) -> TurnCost:
        ...
        if cost.total_nusd > auth.reserved_nusd:
            self._overruns += 1
            logger.warning(
        ...
        with self.player._lock:
            if self._open.pop(auth.auth_id, None) is None:
                raise BudgetMisuseError(
                    ...
                    f"{self.session_id} — already settled, or issued elsewhere"
                )
            cost = price_turn(usage, self.book, plan)
            self._reserved -= auth.reserved_nusd
            self.player._reserved -= auth.reserved_nusd
    ...
    def _record_locked(self, cost: TurnCost) -> None:
        ...
        if self.book.has(CostTier.CLOUD_CHEAP):
            self._shadow_cloud += price_turn(
                cost.usage, self.book, TierPlan.uniform(CostTier.CLOUD_CHEAP)
            ).total_nusd
    ...
    @staticmethod
    def _safe_notify(call: object) -> None:
    ...
    def close(self) -> SessionSummary:
        """Reclaim any leaked reservations and summarize.

        A non-zero ``leaked_authorizations`` is a caller bug — a turn that
        never settled or abandoned — and should fail CI, but must not crash a
        shipped game.
        """
        with self.player._lock:
            leaked = len(self._open)
            for auth in self._open.values():
                self._reserved -= auth.reserved_nusd
                self.player._reserved -= auth.reserved_nusd
            self._open.clear()
            self._closed = True
        if leaked:
            logger.warning(
                "session %s closed with %d unsettled authorization(s)",
                self.session_id,
                leaked,
            )
...
class TierRouter:
    ...
    def plan(
        ...
        *,
        ...
        for tier in sorted(self._candidates(), reverse=True):
            if tier > requested:
                continue
            candidate = (
                TierPlan.local_tts(tier)
                if prefer_local_tts and tier >= CostTier.CLOUD_CHEAP
                else TierPlan.uniform(tier)
            )
